...
async def start_send_products(message: types.Message):
    ...
    button_all = InlineKeyboardButton(text= 'Вывести все товары',
                                      callback_data='all_edit')
    button_one = InlineKeyboardButton(text='Вывести по одному',
...
async def send_all_products(call: types.CallbackQuery):
    products = bot_db.fetch_all_products()
...
async def set_new_value(message: types.Message, state: FSMContext):
    ...
    bot_db.update_product_field(product_id, field, new_value)
...
async def set_new_photo(message: types.Message, state: FSMContext):
    ...
    bot_db.update_product_field(product_id, 'photo', photo_id)
